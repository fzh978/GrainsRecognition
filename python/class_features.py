...
class my_Features(object):
	...
	def calc_features_for_itemlist(self):
		...
		for item in self.itemlist:
			...
			# Hue
			this_feature.append(get_Hue(img.get_img(), img.get_img_binary())[0])
			this_feature.append(get_Hue(img.get_img(), img.get_img_binary())[1])
			# Hu(1)
			myshape=my_SHAPE(img.get_img_gray())
			Hu=myshape.get_humoments()
			this_feature.append(Hu[0])
			this_feature.append(Hu[1])
			'''
			# LBP
